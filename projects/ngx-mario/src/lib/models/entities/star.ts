...
export class Star extends Entity {
    ...
    constructor(pos: [number, number]) {
        super({
            pos,
            sprite: Sprites.get(SpriteType.star),
            hitbox: [0, 0, 16, 16]
        });
    }
    ...
    override checkCollisions(): void {
        ...
        for (let i = 0; i < h; i++) {
            for (let j = 0; j < w; j++) {
                const staticBlock = Game.level.statics[baseY + i]?.[baseX + j];
                const dynamicBlock = Game.level.blocks[baseY + i]?.[baseX + j];

                staticBlock?.isCollideWith(this);
                dynamicBlock?.isCollideWith(this);
            }
        }
    ...
    isPlayerCollided(): void {
        ...
        if (
            !(hpos1[0] > hpos2[0] + Game.player.hitbox[2] || hpos1[0] + this.hitbox[2] < hpos2[0]) &&
            !(hpos1[1] > hpos2[1] + Game.player.hitbox[3] || hpos1[1] + this.hitbox[3] < hpos2[1])
        ) {
            Game.player.star(this.idx!);
        }
    }
